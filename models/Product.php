 ...
 * @property integer $discount_one_c
...
class Product extends \yii\db\ActiveRecord
{
	...
	public $cont;
	public $stock_sum = -1; //for sum stocks
    /**
     * @inheritdoc
     */
	...
	public function sendFoundCheaperMail($url,$phone,$name){
        $receiverEmail = \app\models\SiteInfo::loadData()->contacts_email;
        Yii::$app
            ->mailer
            ->compose(
                ['html' => 'foundCheaper-html', 'text' => 'foundCheaper-text'],
                ['url' => $url, 'phone' => $phone, 'name' => $name]
            )
            ->setFrom([Yii::$app->params['supportEmail'] => Yii::$app->name])
            ->setTo($receiverEmail)
            ->setSubject("Найшли дешевше?")
            ->send();
        return true;
    }
    ...
    {
        return [
            [['category_id', 'manufacturer_id', 'currency_id', 'is_in_stock', 'is_new', 'is_over', 'price_specify','is_suspended','under_the_order','is_on_sale', 'status', 'delivery', 'super', 'topsale', 'is_seeds', 'b_friday'], 'integer'],
            [['updated_at','created_at'], 'safe'],
            [['name_uk', 'name_ru', 'currency_id', 'price'], 'required'],
            [['description_uk', 'description_ru','description_roz','ykazatel','opt','opt_uk','opt1','opt_uk1','dv','dv_uk','code1c'], 'string'],
            ...
            [['discount', 'discount_one_c'], 'integer', 'min' => 1, 'max' => 99],
    ...
    public function attributeLabels()
    {
        return [
			...
			'description_roz' => 'Опис для rozetka',
            'currency_id' => 'Валюта',
            'ykazatel' => 'Указатель',
            'price' => 'Вартість',
            ...
            'discount' => 'Знижка (%)',
			'discount_one_c' => 'Знижка з врахуванням 1с (%)',
    ...
    public function getCurrencyPriceForAttribute()
    {
        $price = (float)$this->getAttributeValues()->one()->price;
        if ($price)
        {
            $price = $this->_convertPrice($price) * $this->getDiscountRate();
        }else{
            $price = $this->getCurrencyPrice();
        }
        return $price;
    }
	
	    public function getOldCurrencyPriceForAttribute()
    {
        $price = (float)$this->getAttributeValues()->one()->price;
        if ($price)
        {
     ...
     * @return float
     */
    public function _convertPrice($price)
    {
	...
	public function orderPayPrice ($amount, $price1c) {
        $_price = $amount * $price1c;
    ...
    {
        return $this->hasMany(Normy::className(), ['product_id' => 'id']);
    }
