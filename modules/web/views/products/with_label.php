...
<div class="items">
	<div class="wrapper">
		...
		<?= $isSale ? '<h3 class="sale-for-client">'.Yii::t('web','Доставка на агромаркет акционных позиций за счет клиента').'</h3>':''?>
		<div class="itemsSidebar" style="width:23%;">
			<?php if ($categories): ?>
				<?= $this->render('/partial/_categories_products_with_label', compact('categories')) ?>
			<?php endif; ?>
					...
					<ul class="itemsSidebar-producers-list">
						...
						<?php foreach ($manufacturers as $manufacturer): ?>
							<li class="itemsSidebar-producers-list-item">
								<a href="<?= Url::toCategory($category, $_manufacturer_ids[$manufacturer['manufacturer']->id]); ?>">
									<label class="itemsSidebar-producers-list-item__label <?=($manufacturer['checked']?' delete':'')?>" >
										<input data-href="<?= Url::toCategory($_manufacturer_ids[$manufacturer['manufacturer']->id], $filter_ids); ?>" type="checkbox" <?=($manufacturer['checked']?'checked':'')?> class="manufacturer_select itemsSidebar-producers-list-item__input"><i> </i>
		...
		<div class="itemsList" style="width:74%;">
			<ul>
				...
				<?php endif; ?><!--<?=$model->super?>-->
                    ...
                    <?php endif; ?>
					<?php if ($model->stock_sum > 0): ?>
                            <div class="itemsList-item-sale__percent" style="width: 100%; text-align: center; color: green; margin-top: 5px;"><?=Yii::t('web', 'Остаток ')?><span><?=(int)$model->stock_sum?></span> <?=Yii::t('web', 'шт')?></div>
                    <?php endif; ?>
                    <?php if ($model->stock_sum == 0): ?>
					...
					<div class="itemsList-item__buy" data-attrs=<?=$model->attributeValues[0]->id?> data-id="<?=$model->id?>"><?=Yii::t('web', 'Купить');?></div>
					</li><?php endforeach; ?>
			</ul>
			<?= LinkPager::widget([
...
$this->registerJs("
$(document).ready(function(){
    if($('body .sale-for-client').hasClass('sale-for-client')){
        $('.items__title').css({
            'height': '50px',
        });
        $('.sale-for-client').css({
