...
$this->title = $model->seoTitle?$model->seoTitle:$model->name . Yii::t('web',' за ') . number_format($model->currencyPriceForAttribute, 2) . Yii::t('web',' грн. купить в Николаеве, Киеве, Одессе, Украине | ') . '(' . Yii::t('web','Арт. ').$model->id .')'. (' | Гектар');
$this->registerMetaTag(['name' => 'description', 'content' => $model->seoDescription?$model->seoDescription:$model->name  . Yii::t('web', ' купить по цене ') . number_format($model->currencyPriceForAttribute, 2) . ' грн (' . Yii::t('web','Арт. ').$model->id .')' . Yii::t('web',' ✔ Гарантия качества ✔ Лучшая цена ✔ Быстрая доставка по Украине ☎ Звоните (067) 559-84-93')], 'description');
...
.btnflip-item.btnflip__center {
  background: linear-gradient(to left, #ff7e19, #ff7e19);
}
      ...
      width: 32%;">
            ...
            <?php if (1 - $model->discountRate): ?>
                        <div class="itemsList-item-sale">
                            ...
                            <div class="itemsList-item-sale__old"><span><?=number_format($model->currencyOldPrice, 2)?></span></div>
            ...
            <?php endif;?>
			<!--
			<a class="btnflip sert-zoom" href="<?=Url::to(['default/save-with-us'])?>" rel="group">
                    <span class="btnflip-item btnflip__front"><?= Yii::t('web', 'Экономьте с нами')?></span>
                    <span class="btnflip-item btnflip__center"></span>
                    <span class="btnflip-item btnflip__back"><?= Yii::t('web', 'Открыть')?></span>
            </a>  -->
            <a class="btnflip sert-zoom" href="#" rel="group" id="found-cheaper">
                <span class="btnflip-item btnflip__front"><?= Yii::t('web', 'Нашли дешевле?')?></span>
                <span class="btnflip-item btnflip__center"></span>
                <span class="btnflip-item btnflip__back"><?= Yii::t('web', 'Открыть')?></span>
            </a>
        </div>
            <div class="item-main">
                ...
                <div class="item-main__id"><?=Yii::t('web','Арт.')?> <?=$model->id?><?=$model->code1c ? '<br/>'.Yii::t('web','Код ').$model->code1c : ''?></div>
                ...
                <form id="form" action="<?=Url::to(['cart/add', 'product_id' => $model->id])?>" method="POST">
                    <div class="item-main-left">
                        ...
                        <div class="item-main-left-additional">
                            <?php if ($model->attributeValues): ?>
                                    ...
                                    <select class="item-main-left-additional__select product_attribute_select" name="attrs[<?=$attributeId?>]">
                                        <?php foreach ($attributeOptions as $attributeValue): ?>
                                            <option class="optPriseCurrent" 
												...
												data-multiplier="<?=$attributeValue->multiplier?>"
                                ...
                                <?php $this->registerJs("
                                    $(document).ready(function(){
                                        ...
                                        text_partner = \"".Yii::t('web','Партнерская цена: ')."\";
										...
										$('.partner_price_placeholder').text(text_partner + $('.product_attribute_select').find('option:first-child').data('partner-price'));
                                        ...
                                        $('.product_attribute_select').change(function() {
											...
											$('.partner_price_placeholder').text(text_partner + $(this).find(':checked').data('partner-price'));
                            ...
                            <input id="countChangeModern" oninput="changetwo()" type="text" class="item-main-left-additional__input" value="1" name="amount">
                    ...
                    <div class="item-main-right">
                        ...
                        <?php endif; ?>
						<?php if ($model->discount_one_c): ?>
                            <div class="item-main-right__rest price-discount-text">
                        ...
                        <span itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                            ...
                            <br>грн<?php if (!$model->ykazatel == '0' && !$model->ykazatel == ''): ?><span class="measure"> / <?=Yii::t('web',$model->ykazatel);?></span><?php endif; ?></div>
							<?php if (Yii::$app->user->identity->clientType->partner_price): ?>
								<div class="item-main-right__rest partner__price">
									<span class="partner_price_placeholder"></span>
									<br>грн<?php if (!$model->ykazatel == '0' && !$model->ykazatel == ''): ?><span > / <?=Yii::t('web',$model->ykazatel);?></span><?php endif; ?>
								</div>
							<?php endif; ?>
                        ...
                        <div class="item-main-right__countModer">
                            <div class="item-main-right__countModer-prise">Кількість</div>
                            <div class="number">
                              <span class="minus">-</span>
                              <input id="countGoods" type="text" value="1" size="2" oninput="change()" />
                              <span class="plus">+</span>
                            </div>
                            <div class="item-main-right__countModer--flex">
                              <div class="item-main-right__countModer-prise item-main-right__countModer-prise--bold">До сплати:</div>
                              <div id="res"></div>
                            </div>
                        </div>
                        <div class="raw type-buy" style="color:white !important;">
						<?php if($model->category->id == 131 || $model->category->parent_id == 124 || $model->category->parent_id == 126
                            ...
                            <button class="btn btn-block" style="word-wrap: break-word; color: #000; margin: 20px 0; text-transform: uppercase; font-size: 12px;" disabled><?=Yii::t('web', 'Продажа только в<br>сети магазинов');?></div></button>
                        <?php else: ?>
                            <button style="color:white !important;" id="buy" class="item-main-right__submit" style="width:88px;"><?=Yii::t('web','Купить')?></button>
                            <button style="color:white !important;;" id="credit" type="button" class="item-main-right__submit" style="background: #f59f08; width:160px;"><?=Yii::t('web','Купить в кредит')?></button>
                            <?php if(Yii::$app->user->identity->clientType->stock_in_cart):?>
                                <button id="shop-stocks" type="button" class="item-main-right__submit" style="background: #f59f08; width:100%; height: 50px;font-size: 12px;line-height: 18px;color:white;"><?=Yii::t('web','Посмотреть остатки на магазинах')?></button> <!--Посмотреть остатки на магазинах-->
                            <?php else: ?>
                ...
                <div id="slideshow-wrap">
                    ...
                    <div id="slideshow-inner" data-slid="<?=$slid?>">
                        ...
                        <?php endif;?>
                            ...
                            <?php if($p['slider'] == 0):?>
                            <?php $mult = (isset($p['attribute']->option->multiplier))?($p['attribute']->option->multiplier/10000):1; ?>
                            <?php $sumcomplect += $p['attribute']->currencyPrice * $mult * (1-$p['discount']/100); ?>
                            <?php $sumcomplectold += $p['attribute']->currencyPrice * $mult; ?>
                            ...
                            <?php if ($p['discount'] > 0) {?>
                                <div class="shares-list-item__price shares-list-item__oldprice"><?=number_format($p['attribute']->currencyPrice * $mult, 2)?> грн</div>
                                <div class="shares-list-item__price shares-list-item__newprice"><?=number_format($p['attribute']->currencyPrice * $mult * (1-$p['discount']/100), 2)?> грн</div>
                            <?php } else { ?>
                                <div class="shares-list-item__price shares-list-item__newprice"><?=number_format($p['attribute']->currencyPrice * $mult, 2)?> грн</div>
        ...
        <?php endif; ?>
            ...
            <div class="tabs">
                <ul class="tabs-control">
                    <li class="tabs-control__item active">
						<h2>
                ...
                <ul class="tabs-list simular-product">
                    ...
                    <li class="tabs-list__item">
                        <ul class="reviews-list">
                            ...
                            <?php /* if (!Yii::$app->user->isGuest && Yii::$app->user->identity->id != $_review->user_id): */ ?>
                                        <span class="reply-comment" data-id="<?=$_review->id ?>" style="font-size: 12px; cursor: pointer">
                            ...
                            <div class="like-value" id="like<?=$_review->id ?>-bs3"><?=$_review->likes ?></div>
                            <i id="dislike<?=$_review->id ?>" data-type="0" data-id="<?=$_review->id ?>" class="glyphicon glyphicon-thumbs-down"></i>
                            <div class="like-value" id="dislike<?=$_review->id ?>-bs3"><?=$_review->dislikes ?></div>
                                    </span>
                                    </span>
                                    ...
                                    <?php $this->context->replyComments($_review->replies,$review,$manufacter, $model); ?>
                            ...
                            <div class="col-md-6 col-xs-12 text-left select-for-mobile">
                                ...
                                <select class="selectpicker for-partner" data-size="7" data-style="btn-info" data-live-search="true" data-none-results-text="<?=Yii::t('web','Не удалось найти н.п.: ')?>{0}">
                                    ...
                                    <?php foreach ($stocksByShops as $stocksByShop):?>
                                        ...
                                        <option data-availability="<?php if($stocksByShop->franch == 0) echo $stocksByShop->main . 'шт. фас. '. $stocksByShop->attributeValue->option->name; else echo $stocksByShop->franch . 'шт. фас. '. $stocksByShop->attributeValue->option->name;?>" value="<?= $stocksByShop->stock1c->representative->id?>"><?= $stocksByShop->stock1c->representative->{'address'.$suff}.' ('. $stocksByShop->attributeValue->option->name.')';?></option>
                                    <?php endforeach;?>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <p class="shop-availability-status" data-availability="1">
                                            (<?php echo $stocksByShop->attributeValue->option->name .') - '; if($stocksByShop->franch == 0) echo $stocksByShop->main . ' шт'; else echo $stocksByShop->franch. ' шт';?>
                                        </p>
                                        <p class="help-status-green" style="display: none"><?php if($stocksByShop->franch == 0) echo $stocksByShop->main; else echo $stocksByShop->franch;?></p>
                                    </div>
                                    <div class="col-md-3 suggested-products">
                                        <a href="#"><p style="color: green;"><?=Yii::t('web','Похожие товары')?></p></a>
                                    </div>
                                </div>
...
<?php endif;?>
    ...
    <div class="modal-overlay-suggested">
    <div class="modal-suggested">

    <a class="close-modal-suggested">
        <svg viewBox="0 0 20 20">
            <path fill="#000000" d="M15.898,4.045c-0.271-0.272-0.713-0.272-0.986,0l-4.71,4.711L5.493,4.045c-0.272-0.272-0.714-0.272-0.986,0s-0.272,0.714,0,0.986l4.709,4.711l-4.71,4.711c-0.272,0.271-0.272,0.713,0,0.986c0.136,0.136,0.314,0.203,0.492,0.203c0.179,0,0.357-0.067,0.493-0.203l4.711-4.711l4.71,4.711c0.137,0.136,0.314,0.203,0.494,0.203c0.178,0,0.355-0.067,0.492-0.203c0.273-0.273,0.273-0.715,0-0.986l-4.711-4.711l4.711-4.711C16.172,4.759,16.172,4.317,15.898,4.045z"></path>
        </svg>
    </a><!-- close modal -->
    <p class="modal-header"><?= Yii::t('web', 'Похожие товары')?></p>
	...
	<div class="modal-overlay-cheaper">
        <div class="modal-cheaper">

            <a class="close-modal-cheaper">
                <svg viewBox="0 0 20 20">
                    <path fill="#000000" d="M15.898,4.045c-0.271-0.272-0.713-0.272-0.986,0l-4.71,4.711L5.493,4.045c-0.272-0.272-0.714-0.272-0.986,0s-0.272,0.714,0,0.986l4.709,4.711l-4.71,4.711c-0.272,0.271-0.272,0.713,0,0.986c0.136,0.136,0.314,0.203,0.492,0.203c0.179,0,0.357-0.067,0.493-0.203l4.711-4.711l4.71,4.711c0.137,0.136,0.314,0.203,0.494,0.203c0.178,0,0.355-0.067,0.492-0.203c0.273-0.273,0.273-0.715,0-0.986l-4.711-4.711l4.711-4.711C16.172,4.759,16.172,4.317,15.898,4.045z"></path>
                </svg>
            </a><!-- close modal -->
            <p class="modal-header"><?= Yii::t('web', 'Нашли дешевле?')?></p>
            <div class = "found-cheaper">
                <p><?=Yii::t('web','Вы нашли точно такой же товар в другом магазине по цене которая ниже нашей? Оставьте ссылку на этот же товар в другом интернет магазине, и свой номер телефона. Мы обязательно сделаем Вам предложение лучше!')?></p>
                <form>
                <div class="form-group row">
                    <label for="cheaper-product" class="col-sm-3 col-form-label"><?=Yii::t('web', 'Ссылка на товар: ')?></label>
                    <div class="col-sm-9">
                        <input type="text" name="url-product" id="cheaper-product" class="form-control" required title="<?=Yii::t('web','Заполните это поле')?>">
                        <div class="invalid-feedback url-check"></div>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="number-cheaper-product" class="col-sm-3 col-form-label"><?=Yii::t('web', 'Номер телефона: ')?></label>
                    <div class="col-sm-9">
                        <input type="text" name="number-cheaper-product" id="number-cheaper-product" class="form-control" required  title="<?=Yii::t('web','Заполните это поле')?>">
                        <div class="invalid-feedback number-check"></div>
                    </div>
                </div>
                <div class="form-group row">
                    <label for="cheaper-click-name" class="col-sm-3 col-form-label"><?=Yii::t('web', 'Имя: ')?></label>
                    <div class="col-sm-9">
                        <input type="text" name="cheaper-click-name" id="cheaper-click-name" class="form-control" required title="<?=Yii::t('web','Заполните это поле')?>">
                        <div class="invalid-feedback name-check"></div>
                    </div>
                </div>
                <div class="col-md-3 col-md-offset-9" style="text-align: right;">
                    <input type="submit" class="btn button-send-cheaper" style="background-color: #ff7e19; color:#fff" value="<?=Yii::t('web','Отправить')?>">
                </div>
                </form>
            </div>
        </div>
...
function inArray(needle, haystack) {
    ...
    $(document).ready(function(){
        $('.message_cancel').on('click', function() {
            jQuery(this).parent().parent().parent().toggleClass('hidden');
        });
        $('.btn-vertical-slider').on('click', function () {
           ...
           var firstOldPrice = 0;
           if(twoParent.find('.item.active .shares-list-item__oldprice') != 'undefined'){
                 firstOldPrice = twoParent.find('.item.active .shares-list-item__oldprice').html();
            }
            ...
            var oldPrice = threeParent.find('.inbuy .shares-list-item__oldprice').html();
           ...
           setTimeout(function(){
            ...
            var secondOldPrice = twoParent.find('.item.active .shares-list-item__oldprice').html();
           ...
           // if(!!oldPrice) oldPrice = '0'; if(!!firstOldPrice) firstOldPrice = '0'; if(!!secondOldPrice) secondOldPrice = '0';
            oldPriceVal.text(String((parseFloat(oldPrice.replace(/,/g,'')) - parseFloat(firstOldPrice.replace(/,/g,'')) + 
            parseFloat(secondOldPrice.replace(/,/g,''))).toFixed(2)).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1,')+ ' грн');
           },700);
		...
		$('.reviews-list-item-header__from, .reply-comment').on('click', function() {
  ...
  $(document).on('ready', function() {  
    $('.modal-shops').on('click','.shop-availability .suggested-products', function(e) {
    
    var product_id = ".$model->id.";
    
    var shop_id = $(this).parent('.row').find('.shop-id').text();
    e.preventDefault();
    $.ajax({
        url: '/web/products/suggested-products-by-shops',
        ...
        success: function(res){
          ...
          if(!res) alert('Ошибка');
                ...
                $('.suggested-product').html(res);
            $('.suggested-product').html(res);
            $('.itemsList-item__img').height(150);
          //  $('.modal-suggested').modal('show');
		var elementsShop = $('.modal-overlay-suggested, .modal-suggested');
            
          elementsShop.addClass('active');  
     

        $('.close-modal-suggested').click(function(){
            elementsShop.removeClass('active');
        });
        $('.modal-overlay-suggested').click(function(e){
           if($(e.target).closest(\".modal-shops\").length==0) elementsShop.removeClass('active');
        });
...
$('#found-cheaper').click(function(e) {
    e.preventDefault();
    var elementsCheaper = $('.modal-overlay-cheaper, .modal-cheaper');   
    elementsCheaper.addClass('active');  
    $('.close-modal-cheaper').click(function(){
            elementsCheaper.removeClass('active');
    });
    $('.modal-overlay-cheaper').click(function(e){
           if($(e.target).closest(\".modal-cheaper\").length==0) elementsCheaper.removeClass('active');
    });
    $('.button-send-cheaper').click(function(e) {
        e.preventDefault();
        var cheaperProduct = $('input#cheaper-product').val();
        if(cheaperProduct == ''){
            $('.url-check').html('".Yii::t('web', 'Вставте, пожалуйста, ссылку на товар')."');
        }else{
           $('.url-check').html(''); 
        }
        var numberCheaperProduct = $('input#number-cheaper-product').val();
        if(numberCheaperProduct == ''){
            $('.number-check').html('".Yii::t('web', 'Заполните поле \"Телефон\"')."');
        }else{
           $('.number-check').html(''); 
        }
        var cheaperClickName = $('input#cheaper-click-name').val();
        if(cheaperClickName == ''){
            $('.name-check').html('".Yii::t('web', 'Заполните поле \"Имя\"')."');
        }else{
           $('.name-check').html(''); 
        }
        $.ajax({
        url: '/web/products/found-cheaper',
           // enctype: 'multipart/form-data',
            data:{url_cheaper: cheaperProduct, number_cheaper_product: numberCheaperProduct, cheaper_click_name: cheaperClickName},
            type: 'POST',
            success: function(res){
...
$(document).ready(function () {
    if($('.btnflip').length > 2){
        $('.btnflip').eq(1).css({'bottom': '20px'});
        $('.btnflip').eq(2).css({'bottom': '-15px'});
    }else{
        $('.btnflip').eq(1).css({'bottom': '20px'});
    }
        $('#number-cheaper-product').inputmask('+38(099)-999-99-99');

});

	$('.header-mid-menu-right-partner').click(function(e){
        $('.tooltip-partner').css({
            'visibility': 'visible',
            'transform': 'translateY(-10px)',
            'opacity': '1',
    ...
    $(document).mouseup(function (e){ // событие клика по веб-документу
		var div = $('.header-mid-menu-right-partner'); // тут указываем ID элемента
		if (!div.is(e.target) // если клик был не по нашему блоку
		    && div.parent().has(e.target).length === 0) { // и не по его дочерним элементам
			$('.tooltip-partner').css({'visibility': 'hidden'}); // скрываем его
		}
	});
