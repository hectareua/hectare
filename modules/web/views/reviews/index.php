    ...
    <div class="reviews">
        ...
        <ul class="reviews-list">
            <?php foreach ($reviews as $i=>$review): ?>
                <li class="reviews-list-item" data-uid="<?=$review->user->id?>" data-ctid="<?= (($review->user->ctype==2)&&($review->user->ctypeid))?($review->user->ctypeid):''?>" style="<?= (($review->user->ctype==2)&&($review->user->ctypeid))?('background:url('.$review->user->manufacturer->logo.') no-repeat 10px 50%;background-size:42px 42px;padding-left: 55px;'):''?>">
                    ...
                    <span class="reply-comment" data-id="<?=$review->id ?>" style="font-size: 12px; cursor: pointer">
                                <img src="/img/reply.png" style="width: 14px; height: 14px">
                        <?=Yii::t('web', 'Ответить')?> </span>
                   <span class="toRight"> <i id="like<?=$review->id ?>" data-type="1" data-id="<?=$review->id ?>" class="glyphicon glyphicon-thumbs-up"></i>
                    <div class="like-value" id="like<?=$review->id ?>-bs3"><?=$review->likes == null ? 0:$review->likes ?></div>
                    <i id="dislike<?=$review->id ?>" data-type="0" data-id="<?=$review->id ?>" class="glyphicon glyphicon-thumbs-down"></i>
                    <div class="like-value" id="dislike<?=$review->id ?>-bs3"><?=$review->dislikes == null ? 0:$review->dislikes ?></div>
                   </span>
                   </span>
        <div class="messagecomment hidden"  id="messagecomment<?=$review->id ?>">
            <?php $form = ActiveForm::begin() ?>        <?php if (Yii::$app->user->identity->ctype == 2){?>
            ...
            <?php ActiveForm::end() ?>
                    ...
                    <?php $this->context->replyComments($review->replies, $model, $manufacter);?>
                </li>
            <?php endforeach; ?>
        </ul>
        <?php $this->registerJs("
...
<style>
.reviews-list-item__content_reply[data-uid="1"], .reviews-list-item[data-uid="2"], .reviews-list-item__content_reply[data-uid="2"] {
    padding-left: 55px;
    background: url(http://hectare.com.ua/images/review_icon.png) no-repeat 10px 50%;
}
</style>
