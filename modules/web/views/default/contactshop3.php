...
//		        timer();
        ...
        $('.bootstrap-select select').change(function(){
            //alert($(this).parent().find('button').text());
            i=infos.length;
            ...
            for (var i = 1; i < infos.length; i++) {        
              ...
              //  alert(info[5]);
                ...
                if (info[3] && info[4].replace(/^\s*/,'').replace(/\s*$/,'') == region) {
                    geocodeAddress(info, geocoder, map);
                }
