            ...
            <?php foreach ($models as $model): ?>
                <?php if(count($model->categories) > 0 || count($model->products) > 0) : ?>
                    <li class="catalog-list-item">
                        <a href="<?=Url::toPage($model)?>" class="catalog-list-item__image" style="background-image:url('<?php if($model->image->url) { echo $model->image->url; }?>')"> </a>
                        <a href="<?=Url::toPage($model)?>" class="catalog-list-item__title"><?=$model->name?></a>
                    </li>
                <?php endif; ?>
            <?php endforeach; ?>
        </ul>
