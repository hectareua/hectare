	...
	//			var_dump($of['currency']["@attributes"]['name'] != "UAH"); echo '<hr>';
		...
		switch ($of["@attributes"]['id']) {
			case "ФР-000015":  //Роздрібна ціна USD
				...
				break;
			case "ФР-000018":  //Роздрібна ціна USD
                $type = 'part_price';
                $price = floatval($price * $course);
                break;
			case "ФР-000017": //Роздрібна ціна EUR":
				...
				$price = floatval($price * $course);
				break;
            case "ФР-000030": //Роздрібна ціна EUR":
                $type = 'part_price';
                $price = floatval($price * $course);
                break;				
			case "ФР-000016": //"Роздрібна ціна UAH":
				$type = 'price';
				break;	
            case "ФР-000019": //"Роздрібна ціна UAH":
                $type = 'part_price';
                break;				
...
//var_dump($offer['categoryPrice']);
	...
	set_time_limit(0);
		...
		if ((strlen($xml)>5) && (count($ar)>0)) {
			...
			switch ($bt) {
				case "main": Stock::deleteAll(['and', ['franch' => 0,'stock' => 0]]); break;
                case "franch": Stock::deleteAll(['and', ['main' => 0, 'stock' => 0]]); break;
                case "stock": Stock::deleteAll(['and', ['main' => 0, 'franch' => 0]]); break;
				default:
			}
			...
			//	var_dump($prod['offer']);echo'<br/><br/>';
				...
				if(!$prodAV) $prodAV = AttributeValue::findOne(['code1c_buh'=>$productId]);
				if ($prodAV) {
					$prodo->sid = $prodAV->id;
